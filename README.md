...
The easiest way is the builder at [airesources.dev/stack/new](https://airesources.dev/stack/new/): pick your tools, and it opens a prefilled pull request for you.
...
Each `stack` entry is a resource id: its path under `src/content/resources/` without the `.md` extension. Only `name` and `stack` are required; the avatar comes from `links.github` unless you set `avatar`. Run `pnpm build` to check the file, then open a pull request.
